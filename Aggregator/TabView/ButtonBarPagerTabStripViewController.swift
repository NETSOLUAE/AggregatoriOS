...
open class ButtonBarPagerTabStripViewController: PagerTabStripViewController, PagerTabStripDataSource, PagerTabStripIsProgressiveDelegate, UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    open override func viewDidLoad() {
        super.viewDidLoad()
        buttonBarItemSpec = .nibFile(nibName: "ButtonBarView", bundle: Bundle(for: ButtonBarViewCell.self), width: { [weak self] (childItemInfo) -> CGFloat in
            let label = UILabel()
    ...
    open func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.heading.text = indicatorInfo.title
        cell.heading.font = UIFont.systemFont(ofSize: 17)
        cell.heading.textColor = settings.style.buttonBarItemTitleColor
        cell.contentView.backgroundColor = settings.style.buttonBarItemBackgroundColor ?? cell.contentView.backgroundColor
        cell.backgroundColor = settings.style.buttonBarItemBackgroundColor ?? cell.backgroundColor
        
        if (isHomeViewDidLoad) {
            if (indexPath.row == 0){
                cell.heading.textColor = UIColor.init(hex: "#ED1941")
            }
        }
